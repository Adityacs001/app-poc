...
import { GETAVAILABLEJOBSEEKERS } from "../../lib/apiendpointconstants";
import { useridentity } from "../../lib/commontypes";

const getavailablejobseekers = async (
  ...
  if (req.method === "GET") {
    ...
    let param: string = `${GETAVAILABLEJOBSEEKERS}?keyword=${keyword}&currentpage=${currentpage}&pagesize=${pagesize}&clientid=${user.clientid}&contactid=${user.contactid}`;
...
export default enhancewithsession(getavailablejobseekers);
