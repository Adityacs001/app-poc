...
import { CLOSEVACANCY } from "../../lib/apiendpointconstants";
...
const closevacancy = async (
  ...
  if (req.method === "POST") {
    const { rid, reasonid, comment } = req.body;
    let param: string = `${CLOSEVACANCY}`;
...
export default enhancewithsession(closevacancy);
