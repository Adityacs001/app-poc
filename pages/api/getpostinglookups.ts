...
import { GETPOSTINGLOOKUPS } from "../../lib/apiendpointconstants";
import { useridentity } from "../../lib/commontypes";

const getpostinglookups = async (
  req: NextApiRequestWithSession,
  res: NextApiResponse,
) => {
  const user = (await req.session.get("user")) as useridentity;
  if (req.method === "GET") {
    let param: string = `${GETPOSTINGLOOKUPS}?clientid=${user.clientid}&contactid=${user.contactid}`;
...
export default enhancewithsession(getpostinglookups);
