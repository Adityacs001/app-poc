...
import { GETACTIVEVACANCYFORMATCHING } from "../../lib/apiendpointconstants";
import { useridentity } from "../../lib/commontypes";

const getactivevacancyformatching = async (
  req: NextApiRequestWithSession,
  res: NextApiResponse,
) => {
  const user = (await req.session.get("user")) as useridentity;
  if (req.method === "GET") {
    let param: string = `${GETACTIVEVACANCYFORMATCHING}?clientid=${user.clientid}&contactid=${user.contactid}`;
...
export default enhancewithsession(getactivevacancyformatching);
