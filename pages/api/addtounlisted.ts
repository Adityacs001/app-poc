...
import { ADDTOUNLISTED } from "../../lib/apiendpointconstants";
import { useridentity } from "../../lib/commontypes";

const addtounlisted = async (
  ...
  if (req.method === "POST") {
    const { rid } = req.body;
    let param: string = `${ADDTOUNLISTED}`;
...
export default enhancewithsession(addtounlisted);
