...
import { GETVACANCYLIST } from "../../lib/apiendpointconstants";
...
const getvacancylist = async (
  ...
  if (req.method === "GET") {
    const { rid } = req.query;
    let param: string = `${GETVACANCYLIST}?clientid=${user.clientid}&contactid=${user.contactid}`;
...
export default enhancewithsession(getvacancylist);
