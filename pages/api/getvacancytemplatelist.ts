...
import { GETVACANCYTEMPLATELIST } from "../../lib/apiendpointconstants";
import { useridentity } from "../../lib/commontypes";

const getvacancytemplatelist = async (
  ...
  if (req.method === "GET") {
    const { rid } = req.query;
    let param: string = `${GETVACANCYTEMPLATELIST}?clientid=${user.clientid}&contactid=${user.contactid}`;
...
export default enhancewithsession(getvacancytemplatelist);
