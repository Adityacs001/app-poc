...
import { GETACTIVEVACANCYFOREXPATS } from "../../lib/apiendpointconstants";
import { useridentity } from "../../lib/commontypes";

const getactivevacancyforexpats = async (
  req: NextApiRequestWithSession,
  res: NextApiResponse,
) => {
  const user = (await req.session.get("user")) as useridentity;
  if (req.method === "GET") {
    let param: string = `${GETACTIVEVACANCYFOREXPATS}?clientid=${user.clientid}&contactid=${user.contactid}`;
...
export default enhancewithsession(getactivevacancyforexpats);
