...
import { GETVACANCYEXPATLIST } from "../../lib/apiendpointconstants";
import { useridentity } from "../../lib/commontypes";

const getvacancyexpatlist = async (
  ...
  if (req.method === "GET") {
    const { rid } = req.query;
    let param: string = `${GETVACANCYEXPATLIST}?rid=${rid}&clientid=${user.clientid}&contactid=${user.contactid}`;
...
export default enhancewithsession(getvacancyexpatlist);
